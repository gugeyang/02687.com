...
* A reverse proxy (Nginx Proxy Manager or Traefik) to handle SSL certificates automatically.
...
Here is the exact `docker-compose.yml` structure I use when prototyping a new department-level LMS deployment. Notice how we isolate the database and cache from public access.
...
If you just spin this up on a raw Ubuntu server, you might hit permission denied errors on the `moodledata_data` volume. Bitnami images run as a non-root user (usually UID 1001). You have to ensure the host directory has the correct ownership. 
...
The fix? Aggressive Redis caching. The `MOODLE_CACHE_DRIVER=redis` variable in the compose file above is not optional for production. It shifts session management and course structure caching directly into RAM.
...
In the next post of this Dev Log, I'll walk through how we can write a custom Python script to automatically synchronize student rosters from a legacy SIS (Student Information System) directly into our new containerized LMS using its REST API.
