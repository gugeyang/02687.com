...
**The sizing formula**: Divide your available RAM (after subtracting ~2GB for the OS and database) by the per-process footprint. Monitor the actual footprint with `ps aux | grep php-fpm | awk '{print $6}' | sort -rn | head` — in my experience, Moodle PHP processes run 60— 0MB each.
